...
public:
    ...
    void union_(const int, const int);
    void printSet();
};
...
void DisjointSet::union_(const int x, const int y) {
...
void testWithoutCompression() {
    ...
    for(int n = 10; n < 1000; n++){
        ...
        begin = clock();
        for(int i = 0; i < n - 1; i++)
            setC->union_(dis(gen),dis(gen));
            setC->findWeighted(dis(gen));
        end = clock();
